from .common import Node, AxisAlignedRule, ExtendedRule, Tree
import numpy as np
...
class ExtendedNode(Node):
    ...
    A node in an isolation forest as originally proposed by Liu
    """
    def __init__(self, extension_level=0, left=None, right=None, rule=None, is_leaf=True, depth=0, size=0):
        """
        Node in an Extended Isolation tree
        :param extension_level: how extended it is, 0 is same as Liu, d-1 where d is the dimensionality
            is fully extended
        :param left: left child
        :param right: right child
        :param rule: rule to determine if left or right child, left is true, right is false
        ...
        :param size: how many data points are at this node (if not completely isolated will be nonzero)
        """
        super(ExtendedNode, self).__init__(content=None, left=left, right=right, rule=rule, is_leaf=is_leaf)
    ...
    def split(self, x, max_depth=25):
        ...
        if x.shape[0] > 1:  # there are data points to split
            ...
            # determine the rule and save it
            chosen_dimensions = np.random.choice(np.arange(dimensionality), replace=False, size=self.extension_level+1)
            low, high = np.min(x[:, chosen_dimensions], axis=0), np.max(x[:, chosen_dimensions], axis=0)
            intercept = np.random.uniform(low, high)
            normal = np.random.uniform(-1, 1, size=self.extension_level+1)
            self.rule = ExtendedRule(normal, intercept, dimensions=chosen_dimensions)

            # make it not a leaf node
            self.is_leaf = False
            self.left = ExtendedNode(extension_level=self.extension_level, depth=self.depth+1)
            self.right = ExtendedNode(extension_level=self.extension_level, depth=self.depth+1)
...
class ExtendedIsolationTree(Tree):
    ...
    An isolation tree as proposed by liu
    """
    def __init__(self, extension_level=None, max_depth=25):
        """
        :param extension_level: None indicates fully extended,
            otherwise extension level is the number of dimensions-1 considered
        :param max_depth: maximum depth a tree can grow to
        """
        super(ExtendedIsolationTree, self).__init__()
        self.extension_level = extension_level
        self.count = 0
        self.max_depth = max_depth
        self.root = ExtendedNode(extension_level=self.extension_level)
...
class ExtendedIsolationForest:
    ...
    An ensemble of Liu Trees
    """
    def __init__(self, tree_count=1, subsample_size=255, max_depth=25, extension_level=None):
        """
        An ensemble of Liu Isolation Trees
        :param tree_count: how many trees to grow
        :param subsample_size: how large the subsample size should be for each
        ...
        self.max_depth = max_depth
        self.extension_level = extension_level
        self.trees = [ExtendedIsolationTree(extension_level=self.extension_level,
                                            max_depth=self.max_depth) for _ in range(tree_count)]
        self.subsample_size = subsample_size
